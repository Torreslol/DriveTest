...
public class Wheel : MonoBehaviour {
	...
	float CalcLongitudinalForce(float Fz,float slip)//纵向的力
	{
	...
	float CalcLateralForce(float Fz,float slipAngle)//侧面的力
	{
		Fz*=0.001f;//convert to kN
	...
	float CalcLongitudinalForceUnit(float Fz,float slip)//纵向的力每单元
	{
		return CalcLongitudinalForce(Fz,slip*maxSlip);
	}
	
	float CalcLateralForceUnit(float Fz,float slipAngle)//侧面的力每单元 
	{
		return CalcLongitudinalForce(Fz,slipAngle*maxAngle);
	}

	Vector3 CombinedForce(float Fz,float slip,float slipAngle)//综合的力
	{
